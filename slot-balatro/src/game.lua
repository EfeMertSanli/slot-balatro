...
function Game.spin()
    ...
    -- Clear any previous calculation display
    CalcDisplay.clear()
...
function Game.check_win()
    ...
    -- Apply joker on_spin effects (always runs)
    ...
    if total_score > 0 then
        -- Start calculation display with expected payline count
        CalcDisplay.start(0, #Game.win_reveal_queue)
        CalcDisplay.set_fast_forward(Game.fast_forward)

        -- Add each payline as a separate step (will be revealed in sync with reels)
        local payline_colors = {
            ...
            [-2] = Colors.blue,
            [2] = Colors.red,
            ["diag_down"] = Colors.orange,
            ["diag_up"] = Colors.purple,
        }

        for i, win_data in ipairs(Game.win_reveal_queue) do
            local payline_name = Game.get_payline_name(win_data.payline)
            local color = payline_colors[win_data.payline] or Colors.cyan
            CalcDisplay.add_payline_step(payline_name, win_data.score, color)
        end

        -- Step 2: Apply joker effects
        local pre_joker_score = total_score
        for _, joker in ipairs(Game.jokers) do
            if joker.apply then
                local new_score = joker.apply(total_score, center_symbols, Game)
                if new_score ~= total_score then
                    local mult = new_score / total_score
                    CalcDisplay.add_step(CalcDisplay.STEP_TYPES.JOKER, joker.name, mult, "x", joker.color or Colors.purple, joker.icon or "JKR")
                    total_score = new_score
                end
            end
        end

        -- Step 3: Apply permanent win bonus
        local win_mult = 1 + (Game.perm_win_bonus or 0)
        if win_mult > 1 then
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.PERM_BONUS, "Win Bonus", win_mult, "x", Colors.gold, "+%")
        end
        total_score = math.floor(total_score * win_mult)

        -- Step 4: Apply payline bonus (bonus per active payline)
        local payline_bonus = (#Game.paylines) * (Game.perm_payline_bonus or 0)
        if payline_bonus > 0 then
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.PERM_BONUS, "Payline Bonus", payline_bonus, "+", Colors.cyan, "PAY")
            total_score = total_score + payline_bonus
        end

        -- Step 5: Apply combo multiplier (consecutive wins)
        local combo_mult = Combo.on_win()
        if combo_mult > 1.0 then
            local combo_info = Combo.get_display_info()
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.COMBO, "Combo x" .. combo_info.wins, combo_mult, "x", Colors.orange, "CMB")
        ...
        -- Step 6: Apply Lucky Coin double win effect
        if ConsumablesSystem.is_double_win_active() then
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.DOUBLE, "Lucky Coin", 2, "x", Colors.gold, "x2")
        ...
        -- Step 7: Apply special event payout multiplier (e.g., Lucky Hour, High Roller)
        local event_mult = SpecialEvents.get_payout_multiplier()
        if event_mult > 1.0 then
            local event = SpecialEvents.get_active()
            local event_name = event and event.name or "Event"
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.EVENT, event_name, event_mult, "x", Colors.magenta, "EVT")
        ...
        -- Step 8: Apply boss on_win modifier (e.g., The Chaos randomizes score)
        local pre_boss_score = total_score
        total_score = BossModifiers.on_win(Game, total_score)
        if total_score ~= pre_boss_score then
            local boss = BossModifiers.get_current()
            local boss_name = boss and boss.name or "Boss"
            local boss_mult = total_score / pre_boss_score
            CalcDisplay.add_step(CalcDisplay.STEP_TYPES.BOSS, boss_name, boss_mult, "x", Colors.red, "BSS")
        end

        -- Final total step
        CalcDisplay.add_step(CalcDisplay.STEP_TYPES.TOTAL, "TOTAL", total_score, "=", Colors.gold, "WIN")

        -- Finalize calculation display
        CalcDisplay.finalize()

        Game.credits = Game.credits + total_score
        Game.last_win = total_score

        -- Play cash sound for credit gain
        Sound.cash()
...
function Game.reveal_next_win()
    ...
    -- Sync with CalcDisplay - reveal this payline's score chip
    CalcDisplay.reveal_payline(Game.win_reveal_index)
    ...
    -- Calculate payline Y position for effects
    ...
    if Game.num_reels > 3 then
        reel_width = math.floor((reel_container_w - 60 - (Game.num_reels - 1) * 20) / Game.num_reels)
        reel_spacing = 20
    end
    local total_reels_w = Game.num_reels * reel_width + (Game.num_reels - 1) * reel_spacing
    local reel_start_x = reel_container_x + (reel_container_w - total_reels_w) / 2

    -- Get reel center X positions
    local reel_positions = {}
    for i = 1, Game.num_reels do
        table.insert(reel_positions, reel_start_x + (i-1) * (reel_width + reel_spacing) + reel_width / 2)
    end

    -- Calculate payline Y based on type
    local payline_y
    local payline_ys = {}  -- For diagonals
    if type(win_data.payline) == "string" then
        -- Diagonal paylines
        local max_offset = math.floor(num_rows / 2)
        for i = 1, Game.num_reels do
            local t = (i - 1) / (Game.num_reels - 1)
            local offset
            if win_data.payline == "diag_down" then
                offset = math.floor(-max_offset + t * max_offset * 2 + 0.5)
            else -- diag_up
                offset = math.floor(max_offset - t * max_offset * 2 + 0.5)
            end
            ...
            table.insert(payline_ys, reel_start_y + row_index * Config.SYMBOL_HEIGHT + Config.SYMBOL_HEIGHT * 0.5)
        end
        payline_y = payline_ys[math.ceil(#payline_ys / 2)]  -- Middle point
    else
        -- Horizontal payline
        local row_index = center_row + win_data.payline
        payline_y = reel_start_y + row_index * Config.SYMBOL_HEIGHT + Config.SYMBOL_HEIGHT * 0.5
    end
    ...
    if is_triple then
        ...
        if win_data.score >= 100 then
            Sound.jackpot()
        elseif win_data.score >= 50 then
            Sound.big_win()
        else
            Sound.win()
        end

        -- Spawn coins at payline position
        if type(win_data.payline) == "string" then
            Effects.spawn_diagonal_coins(reel_positions, payline_ys, 6)
        else
            Effects.spawn_payline_coins(reel_positions, payline_y, 6)
        end

        Effects.trigger_win_effects(win_data.score, win_data.win_type, cx, payline_y)
    else
        Game.win_flash = Game.fast_forward and 0.15 or 0.4

        -- Play coin sound for smaller wins
        Sound.coin()

        -- Spawn fewer coins at payline position
        if type(win_data.payline) == "string" then
            Effects.spawn_diagonal_coins(reel_positions, payline_ys, 3)
        else
            Effects.spawn_payline_coins(reel_positions, payline_y, 3)
        end
    end
...
function Game.is_calc_display_active()
    return CalcDisplay.is_active()
end

function Game.get_calc_steps()
    return CalcDisplay.get_revealed_steps()
end

function Game.get_calc_running_total()
    return CalcDisplay.get_running_total()
end

function Game.is_calc_complete()
    return CalcDisplay.is_complete()
end

function Game.skip_calc_display()
    CalcDisplay.skip_to_end()
end
