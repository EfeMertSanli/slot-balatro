...
local crt_shader_code = [[
    ...
    vec4 effect(vec4 color, Image tex, vec2 tc, vec2 sc) {
        ...
        // HEAVY scanlines - every 2nd row much darker (doubled from 0.5 to 1.0)
        float scanlineY = floor(uv.y * screen_size.y);
        float scanline = mod(scanlineY, 2.0);
        float scanlineStrength = logScale * 1.0;  // 100% darkening on alt rows
        texColor.rgb *= 1.0 - scanline * scanlineStrength * 0.7;  // Cap at 70% to keep visible

        // Strong brightness compensation to keep it bright
        float scanlineCompensation = 1.0 + scanlineStrength * 0.8;
        texColor.rgb *= scanlineCompensation;

        // Heavy phosphor dot pattern (RGB subpixels) - doubled strength
        float pixelX = floor(uv.x * screen_size.x);
        float subpixel = mod(pixelX, 3.0);
        float phosphorStrength = logScale * 0.16;
        if (subpixel < 1.0) {
            texColor.r += phosphorStrength;
            texColor.gb -= phosphorStrength * 0.3;  // Color bleeding
        } else if (subpixel < 2.0) {
            texColor.g += phosphorStrength;
            texColor.rb -= phosphorStrength * 0.3;
        } else {
            texColor.b += phosphorStrength;
            texColor.rg -= phosphorStrength * 0.3;
        }

        // Heavy ordered dithering (Bayer 4x4 pattern) - doubled
        vec2 pixelPos = floor(uv * screen_size);
        float bayerValue = bayer4x4(pixelPos);
        float ditherStrength = logScale * 0.16;
        texColor.rgb += (bayerValue - 0.5) * ditherStrength;

        // Strong vignette (darkens edges) - doubled with compensation
        vec2 center = uv - 0.5;
        float vignetteStrength = dot(center, center) * 2.0 * logScale;
        float vignette = 1.0 - vignetteStrength;
        float vignetteCompensation = 1.0 + vignetteStrength * 0.6;
        texColor.rgb *= vignette * vignetteCompensation;

        // Stronger flicker for CRT feel
        float flickerStrength = 0.04 / (0.5 + logScale);
        float flicker = 1.0 - sin(time * 60.0) * flickerStrength;
        texColor.rgb *= flicker;

        // Chromatic aberration at edges (CRT color fringing)
        float aberrationStrength = dot(center, center) * logScale * 0.008;
        texColor.r = Texel(tex, uv + vec2(aberrationStrength, 0.0)).r * 0.3 + texColor.r * 0.7;
        texColor.b = Texel(tex, uv - vec2(aberrationStrength, 0.0)).b * 0.3 + texColor.b * 0.7;

        // Slight barrel distortion simulation via color shift
        float barrelStrength = dot(center, center) * logScale * 0.1;
        texColor.rgb *= 1.0 + barrelStrength * 0.2;
...
local function create_all_buttons()
    ...
    -- Play screen buttons - positioned in action bar (centered between side panels)
    local action = Config.LAYOUT.action_bar
    local play_btn_y = action.y + 50
    local btn_h = 70

    local spin_w = 260
    local small_btn_w = 80
    local shop_btn_w = 120
    local paytable_btn_w = 120
    local spacing = 15
    ...
    -- Fast forward button (below main buttons)
    local ff_btn_y = play_btn_y + btn_h + 15
    local ff_btn_w = 140
    local ff_btn_x = action.x + (action.w - ff_btn_w) / 2
...
function love.load()
    ...
    -- Initialize sound system and play music
    Sound.load()
    Sound.play_music(1)
...
function love.keypressed(key)
    ...
    elseif key == "-" or key == "kp-" then
        -- Lower music volume
        Sound.set_music_volume(Sound.get_music_volume() - 0.1)
    elseif key == "=" or key == "kp+" then
        -- Raise music volume
        Sound.set_music_volume(Sound.get_music_volume() + 0.1)
    end
